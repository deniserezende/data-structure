...
// Function that verifies whether a point is inside the circle or in it's border, if so returns 1.
int point_in_circle(type_circle circle, double px, double py);

// Function that verifies whether a circle is inside another one
...
// Function that verifies whether a circle center is inside another one
// Note that: if the circles are exactly the same the function considers the center inside the reference circle.
int circle_center_in_circle(type_circle reference_circle, type_circle second_circle);

// Function that verifies whether a circle is completely inside a rect.
// Note that: the circle is considered inside the rect even if their borders are colliding.  
int circle_in_rect(type_rect rect, type_circle circle);

// Function that verifies whether the center of a circle is inside a rect.
int circle_center_in_rect(type_rect rect, type_circle circle);

// Function that verifies whether a point is in a segment.
int point_in_segment(type_segment segment, double px, double py);

// Function that verifies whether it exists or not the intersection of two given segments.
...
// Similar to the above, only difference is the parameters given.
