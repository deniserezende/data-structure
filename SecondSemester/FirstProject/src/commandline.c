...
void commandline(int argc, char *argv[], char **input_path, char **geo_filename, char **qry_filename, char **pm_filename, char **output_path){
        ...
        if(strcmp("-e", argv[i]) == 0){
            ...
            *geo_filename = malloc((strlen(argv[i])+1) *sizeof(char));
            sprintf(*geo_filename, "%s", argv[i]); // Copies the value of argv
                    ...
                    *pm_filename = malloc((strlen(argv[i])+1) *sizeof(char));
                    sprintf(*pm_filename, "%s", argv[i]); // Copies the value of argv
                    }
