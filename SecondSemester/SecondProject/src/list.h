...
// Post-condition: if item exists current is set to this item, otherwise current remains where it was.
// Returns a boolean: True if the item was found and false otherwise. 
// More accurate because compares the actual data and not only pointers 
int find_item_in_list_with_compare(type_list list, type_litems item, type_lptrf_twoitems compare);
